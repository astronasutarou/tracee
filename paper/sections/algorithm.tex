...
In the first part of the algorithm, we construct a $k$-nearest neighbor graph ($k$-NN graph) from the vertices. In the $k$-nearest neighbor graph, every vertex has at least $k$ edges to the $k$th nearest vertices. Samples of the $k$-NN graph are presented in Figure~\ref{fig:algo:knn_sample}. The blue dots represent the vertices. The solid and dashed lines show the $k$-NN graphs for $k=1$ and $3$, respectively.
